...
function addAccessors($scope) {
  ...
  $scope.confirmClear = function() {
    if (confirm('¿Desea borrar la imagen actual?')) {
      canvas.clear();
    }
  };

  $scope.confirmClearMasks = function() {
    if (confirm('¿Desea borrar todos los trazos?')) {
...
$scope.updateCanvas = function () {
    fabric.Image.fromURL(output_canvas.toDataURL('png'), function(oImg) {
        oImg.height = output_canvas.height;
        oImg.width = output_canvas.width;
        canvas.add(oImg);
    });
};
...
$scope.segment = function () {
    ...
    if (state.masks_present) {
        $scope.status = "Starting segmentation";
        if(canvas.isDrawingMode){
